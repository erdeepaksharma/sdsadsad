...
<ul class="seaocore_sidebar_list">
  <?php foreach ($this->paginator as $sitereview_video): ?>
    <?php
    $this->partial()->setObjectKey('sitereview_video');
    ...
    $tmpBody = strip_tags($sitereview_video->getParent()->getTitle());
    $listing_title = ( Engine_String::strlen($tmpBody) > $truncation_limit ? Engine_String::substr($tmpBody, 0, $truncation_limit) . '..' : $tmpBody );
    ?>
    <?php echo $this->translate("in ") . $this->htmlLink($sitereview_video->getParent()->getHref(), $listing_title, array('title' => $sitereview_video->getParent()->getTitle())) ?>    
  </div>
