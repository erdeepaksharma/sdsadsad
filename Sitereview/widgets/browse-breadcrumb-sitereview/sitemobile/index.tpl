...
<?php if(!empty($this->category_id) || (isset($this->formValues['tag']) && !empty($this->formValues['tag']) && isset($this->formValues['tag_id']) && !empty($this->formValues['tag_id']))): ?>
	<div class="sr_listing_breadcrumb">
      ...
      <?php echo $this->htmlLink($this->url(array('action' => 'index'), "sitereview_general_listtype_$this->listingtype_id"), $this->translate("Browse $this->listing_plural_uc")) ?>
          ...
          <?php if ($this->subcategory_name != ''):?> 
            ...
            <?php echo $this->htmlLink($this->url(array('category_id' => $this->category_id, 'categoryname' => Engine_Api::_()->getItem('sitereview_category', $this->category_id)->getCategorySlug()), "sitereview_general_category_listtype_$this->listingtype_id"), $this->translate($this->category_name)) ?>
          <?php else: ?>
          ...
          <?php endif; ?>
			...
			<?php if(((isset($this->formValues['tag']) && !empty($this->formValues['tag']) && isset($this->formValues['tag_id']) && !empty($this->formValues['tag_id'])))): ?>
				<?php $tag_value = $this->formValues['tag']; $tag_value_id = $this->formValues['tag_id']; $browse_url = $this->url(array('action' => 'index'), "sitereview_general_listtype_$this->listingtype_id", true)."?tag=$tag_value&tag_id=$tag_value_id";?>
        ...
        <?php if($this->current_url2): ?>  
          <a href="<?php echo $this->url(array( 'action' => 'index'), "sitereview_general_listtype_$this->listingtype_id", true)."?".$this->current_url2; ?>"><?php echo $this->translate('(x)');?></a></b>
        <?php else: ?>
          <a href="<?php echo $this->url(array( 'action' => 'index'), "sitereview_general_listtype_$this->listingtype_id", true); ?>"><?php echo $this->translate('(x)');?></a></b>        
        <?php endif; ?>
			<?php endif; ?>
	</div>
<?php endif; ?>
