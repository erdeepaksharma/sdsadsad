  ...
  <script type="text/javascript">
   ...
   if($("pinboard_<?php echo $this->identity ?>").getParent('.layout_left')){
    ...
    PinBoardSeaoObject[layoutColumn].add({
      ...
      responseContainerClass :'layout_sitereview_pinboard_browse'
    });
  ...
  <?php foreach ($this->paginator as $sitereview): ?>
    ...
    <?php echo $this->timestamp(strtotime($sitereview->creation_date)) ?>
          ...
          <div class="seaocore_board_list_cont">
            ...
            <?php endif;  ?>
            <?php if(!empty($sitereview->price) && $sitereview->price > 0 && Zend_Registry::get('listingtypeArray' . $sitereview->listingtype_id)->price  && isset($this->params['showPrice']) && $this->params['showPrice']): ?>
             ...
             <?php endif; ?>
                        <?php if(isset($this->params['showLocation']) && $this->params['showLocation'] && Zend_Registry::get('listingtypeArray' . $sitereview->listingtype_id)->location  && !empty($sitereview->location)):?>
            <div class="seaocore_stats seaocore_txt_light mtop5">
...
<?php else: ?>
    ...
    <?php if ($this->currentpage < 2): ?>
        <div class="tip">
            <span>
                <?php echo $this->translate('Nobody has posted a '.strtolower($this->listingtypeArray->title_singular).' with that criteria.'); ?>
            </span>
        </div>
    <?php endif; ?>
<?php endif; ?>
