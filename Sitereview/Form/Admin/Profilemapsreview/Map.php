 ...
 */
class Sitereview_Form_Admin_Profilemapsreview_Map extends Engine_Form {
  ...
  public function init() {
    ...
    $this->setMethod('post')
            ...
            ->setDescription("After selecting a profile type, if you click on 'Save', then the already created reviews for listings of this category will also be associated with this profile type.");
    ...
    if (!empty($getCountChieldMapping)) {
      ...
      if ($category->cat_dependancy == 0 && $category->subcat_dependancy == 0) {
        $description = Zend_Registry::get('Zend_Translate')->_("<div class='tip'><span>Note: It seems that sub-categories of this category have been already mapped. Thus, if you add this mapping, then earlier reviews for listings associated with the mapped sub-categories will loose their custom profile data.</span></div>");
      } else {
        $description = Zend_Registry::get('Zend_Translate')->_("<div class='tip'><span>Note: It seems that 3rd level categories of this sub-category have been already mapped. Thus, if you add this mapping, then earlier reviews for listings associated with the mapped 3rd level categories will loose their custom profile data.</span></div>");
      }
    ...
    if (count($topStructure) == 1 && $topStructure[0]->getChild()->type == 'profile_type') {
      ...
      if (count($options) > 0) {
        $options = $profileTypeField->getElementParams('sitereview_review');
