...
<?php if ($this->sizeType == 'small-star'): ?>
  ...
  <span class="list_rating_star" title="<?php echo $html_title; ?>">
      ...
      <?php for ($x = 1; $x <= $this->rating; $x++): ?>
        <span class="rating_star_generic <?php echo $this->rating_star_class ?>" ></span>
      <?php endfor; ?>
      <?php $roundrating = round($this->rating); ?>
      <?php if (($roundrating - $this->rating) > 0): ?>
        <span class="rating_star_generic <?php echo $this->rating_half_star_class ?>" ></span>
      <?php endif; ?>
      <?php $roundrating++; ?>
...
<?php else: ?>
  <?php
$this->headLink()
	->prependStylesheet($this->layout()->staticBaseUrl.'application/modules/Sitereview/externals/styles/star-rating.css');
  ?>
