  ...
  <?php echo $this->partial('application/modules/Sitereview/views/scripts/dashboard/header.tpl', array('sitereview' => $this->sitereview)); ?>
	<div class="sr_create_list_form">
		<div class="global_form">
			<div>
				<div>
				<h3> <?php echo $this->translate("Edit $this->listing_singular_uc Videos"); ?></h3>
				<p class="form-description"><?php echo $this->translate("Edit and manage the videos of your $this->listing_singular_lc below."); ?>
					<?php if($this->slideShowEnanle):?>
            <br />
						<?php echo $this->translate("An attractive Slideshow will be displayed on your $this->listing_singular_uc Profile page. Below, you can select a video to be displayed in that slideshow by using the 'Show in Slideshow' option."); ?>
            <br />
      ...
      <?php if($this->upload_video):?>
        ...
        <?php if($this->type_video):?>                 
            <?php echo $this->htmlLink(array('route' => "sitereview_video_upload_listtype_$this->listingtype_id", 'action' => 'index', 'listing_id' => $this->sitereview->listing_id), $this->translate('Add New Video'), array('class' => 'buttonlink icon_sitereviews_video_new')) ?>
        <?php else:?>
            <?php echo $this->htmlLink(array('route' => "sitereview_video_create_listtype_$this->listingtype_id", 'listing_id' => $this->sitereview->listing_id), $this->translate('Add New Video'), array('class' => 'buttonlink icon_sitereviews_video_new')) ?>
            <?php endif;?>
        <?php else:?>       
                                    
            <?php if (Engine_Api::_()->sitevideo()->openPostNewVideosInLightbox()): ?>                         
            <?php echo $this->htmlLink(array('route' => "sitevideo_video_general", 'action' => 'create',  'parent_type' => $this->sitereview->getType() . '_' . $this->listingtype_id, 'parent_id' => $this->sitereview->getIdentity()), $this->translate('Add New Video'), array('data-SmoothboxSEAOClass' => 'seao_add_video_lightbox',  'class' => 'seao_smoothbox buttonlink icon_sitereviews_video_new')) ?>
                <?php else:?>
                            <?php echo $this->htmlLink(array('route' => "sitevideo_video_general", 'action' => 'create',  'parent_type' => $this->sitereview->getType() . '_' . $this->listingtype_id, 'parent_id' => $this->sitereview->getIdentity()), $this->translate('Add New Video'), array('class' => 'buttonlink icon_sitereviews_video_new')) ?>        
                 <?php endif;?>                   
      ...
      <?php endif;?>
				...
				<form action="<?php echo $this->escape($this->form->getAction()) ?>" method="<?php echo $this->escape($this->form->getMethod()) ?>" class="">
					<div>
						<div>
							<ul class='sr_edit_media' id="video">
								<?php if(!empty($this->count)): ?>
					              ...
					              <?php 
					                if ($item->photo_id)
					                  echo $this->htmlLink($item->getHref(array('listingtype_id' => $this->listingtype_id,'content_id' => $this->content_id)), $this->itemPhoto($item, 'thumb.normal'), array());
					                else
				          ...
				          <?php endforeach; ?>
								<?php else:?><br />
									...
									<div class="tip">
										...
										<span>
											<?php if($this->type_video):?>
												<?php $url = $this->url(array('action' => 'create',  'parent_type' => $this->sitereview->getType() . '_' . $this->listingtype_id, 'parent_id' => $this->sitereview->getIdentity(), 'content_id' => $this->identity), "sitevideo_video_general", true);?>
												<?php echo $this->translate('You have not added any video in your '.$this->listing_singular_lc.'. %1$sClick here%2$s to add your first video.', "<a class='seao_smoothbox' data-SmoothboxSEAOClass='seao_add_video_lightbox' href='$url'>", "</a>"); ?>
											<?php else:?>
											<?php $url = $this->url(array('action' => 'create',  'parent_type' => $this->sitereview->getType() . '_' . $this->listingtype_id, 'parent_id' => $this->sitereview->getIdentity(), 'content_id' => $this->identity), "sitevideo_video_general", true);?>
												<?php echo $this->translate('There are currently no videos in this '.$this->listing_singular_lc.'. %1$sClick here%2$s to add your first video.', "<a href='$url'>", "</a>"); ?>
											<?php endif;?>
