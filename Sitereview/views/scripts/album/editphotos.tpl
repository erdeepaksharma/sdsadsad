		...
		<div class="global_form">
			<div>
				<div>
					...
					<p class="form-description"><?php echo $this->translate("Edit and manage the photos of your $this->listing_singular_lc below."); ?>
	        ...
	        <?php if(!empty($this->upload_photo)):?>
						<div class="clr">
							<?php echo $this->htmlLink(array('route' => "sitereview_photoalbumupload_listtype_".$this->sitereview->listingtype_id,'album_id' => $this->album_id, 'listing_id' => $this->listing_id), $this->translate('Add New Photos'), array('class' => 'buttonlink icon_photos_new')) ?>
						</div>
				  <?php endif;?>
					...
					<form action="<?php echo $this->escape($this->form->getAction()) ?>" method="<?php echo $this->escape($this->form->getMethod()) ?>">
						...
						<ul class='sr_edit_media' id="photo">
							<?php if(!empty($this->count)): ?>
								<?php foreach ($this->paginator as $photo):?>
									<li>
										<div class="sr_edit_media_thumb"> <?php echo $this->itemPhoto($photo, 'thumb.normal') ?> </div>
