 ...
 * @license    http://www.socialengine.com/license/
 */
class Advancedactivity_Plugin_Composer extends Core_Plugin_Abstract
{
  public function onAttachLink($data)
  {
    try {
      ...
      if(class_exists('Engine_Iframely') && !empty($iframelyConfig['host']) && $iframelyConfig['host'] != 'none' ) {
        $response = Engine_Iframely::factory($iframelyConfig)
          ->get($data['uri']);
          $data['params']['iframely'] = $response ? $response : array();
      }
      $link = Engine_Api::_()->getApi('links', 'advancedactivity')->createLink($viewer, $data);
    } catch( Exception $e ) {
      throw $e;
      return;
    }
